...
def test_add_group(app, db, json_groups):
    with pytest.allure.step('Get new Group'):
        group = json_groups
    with pytest.allure.step('Get old Groups'):
        old_groups = db.get_group_list()
    with pytest.allure.step('Create Group with name %s' % group):
        app.group.create(group)
    with pytest.allure.step('Check Group'):
...
def test_edit_group(app, db, json_groups, check_ui):
    with pytest.allure.step('Get new Group'):
        group_j = json_groups
    with pytest.allure.step('Check count Group'):
        if len(db.get_group_list()) == 0:
            app.group.create(group_j)
    with pytest.allure.step('Get list old Groups and choice'):
        old_groups = db.get_group_list()
        group = random.choice(old_groups)
    with pytest.allure.step('Edit Group with name %s' % group):
